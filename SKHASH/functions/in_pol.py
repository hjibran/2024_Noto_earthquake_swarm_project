...
def read_quakeml_polarity_file(fpfile,p_weight_I=1.0,p_weight_E=0.5):
	...
	if (~pd.isnull(pick_df['polarity']) & pd.isnull(pick_df[['takeoff','azimuth','sr_dist_km']]).any(axis=1)).any():
		...
		pol_pick_df[['azimuth','takeoff','sr_dist_km']]=(pol_pick_df.merge(ta_pick_df[['network','station','location','azimuth','takeoff','sr_dist_km']],on=['network','station','location'],how='left'))[['azimuth_y','takeoff_y','sr_dist_km_y']].values
